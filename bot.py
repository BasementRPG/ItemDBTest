...
async def ensure_maps_table():
    ...
    async with db_pool.acquire() as conn:
        ...
        # ----------------------------------------------------
        # Add map_number to an older maps table if necessary
        # ----------------------------------------------------
        await conn.execute("""
            ALTER TABLE maps
        ...
        # ----------------------------------------------------
        # Existing maps from the old system did not have a
        # map_number. Give them map number 1.
        # ----------------------------------------------------
        await conn.execute("""
            UPDATE maps
            SET map_number = 1
        ...
        # ----------------------------------------------------
        # Remove the OLD unique constraint that allowed only
        # one map per zone.
        #
        # This is the normal PostgreSQL name generated by:
        ...
        # ----------------------------------------------------
        # Make sure map numbers are unique within each guild
        # and zone.
        # ----------------------------------------------------
        await conn.execute("""
...
def create_map_embed(zone_name, map_number, map_image):
    """
    Create the embed used when displaying a map.
    """
...
class MapsZoneSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Reset selected map whenever the zone changes
        self.parent_view.selected_map_number = None

        # Load maps for this zone
        await self.parent_view.load_maps(selected_zone)

        await interaction.response.edit_message(
            content=f"🗺️ **{selected_zone}**\nSelect a map:",
            embed=None,
...
class MapsNumberSelect(discord.ui.Select):
    ...
    def __init__(self, parent_view):
        ...
        for map_number in parent_view.map_numbers:

            options.append(
                discord.SelectOption(
                    label=f"Map {map_number}",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "none":
            ...
            await interaction.response.send_message(
                "❌ No maps are available for this zone.",
                ephemeral=True
            )
            return

        self.parent_view.selected_map_number = int(self.values[0])

        await self.parent_view.show_selected_map(interaction)
...
class MapsView(discord.ui.View):
    ...
    def __init__(self, interaction: discord.Interaction, zones):
        ...
        self.selected_map_number = None

        self.map_numbers = []

        # Add zone selector
        self.add_item(MapsZoneSelect(self))

        # Buttons are declared below

        self.update_buttons()

    # --------------------------------------------------------
    # Load maps for selected zone
    # --------------------------------------------------------

    async def load_maps(self, zone_name):
        ...
        async with db_pool.acquire() as conn:
            ...
            """,
            self.original_interaction.guild.id,
            zone_name)
        ...
        # Remove old map-number selector
        for child in list(self.children):

            if isinstance(child, MapsNumberSelect):
                self.remove_item(child)

        # Add new map-number selector
        self.add_item(MapsNumberSelect(self))
    ...
    # --------------------------------------------------------
    # Refresh zone selector when changing pages
    # --------------------------------------------------------
    ...
    async def send_privately_button(
        ...
        if not self.selected_map_number:
        ...
        async with db_pool.acquire() as conn:
            ...
            row = await conn.fetchrow("""
                SELECT zone_name, map_number, map_image
                FROM maps
                WHERE guild_id = $1
                  AND zone_name = $2
                  AND map_number = $3
            ...
            self.selected_zone,
            self.selected_map_number)

        if not row:

            await interaction.response.send_message(
                "❌ That map could not be found.",
                ephemeral=True
            )
            return
    ...
    # --------------------------------------------------------
    # Display selected map in the main message
    # --------------------------------------------------------

    async def show_selected_map(self, interaction):
        ...
        await interaction.response.edit_message(
            content=f"🗺️ **{row['zone_name']} - Map {row['map_number']}**",
            embed=embed,
            view=self
        )
...
async def mapadd(
    ...
    # Keep zone capitalization consistent
    zone_name = format_item_name(zone_name)
    ...
    try:
        ...
        # ----------------------------------------------------
        # Upload image to the same hidden channel used by
        # the item database.
        # ----------------------------------------------------
        ...
        await interaction.edit_original_response(
            content=f"✅ **{zone_name} - Map {next_map_number}** was added.",
            embed=embed
        )

    except discord.Forbidden:

        # Cleanup uploaded message if possible
        if uploaded_message:
        ...
        # ----------------------------------------------------
        # If DB insert failed after Discord upload, delete the
...
class ConfirmMapRemoveView(discord.ui.View):
    ...
    async def confirm(
        ...
        try:
            ...
            # ------------------------------------------------
            # Delete uploaded Discord message
            # ------------------------------------------------
            ...
            async with db_pool.acquire() as conn:
                ...
                async with conn.transaction():
                    ...
                    # ----------------------------------------
                    # Renumber remaining maps for this zone
                    #
                    # Example:
                    #
                    # 1
            ...
            image_status = (
                "Image deleted from upload channel."
                if image_deleted
                else
                "⚠️ Database entry removed, but I could not delete "
                "the image from the upload channel."
            )
            ...
            await interaction.response.edit_message(
                content=(
                    ...
                    f"{image_status}\n"
                    f"Remaining maps for this zone were renumbered."
                ),
                view=None
            )
...
async def mapremove(
    ...
    # --------------------------------------------------------
    # Verify map exists before showing confirmation
    # --------------------------------------------------------
